...
obsloop: do    ! no end limit - have the loop break when input ends
   ...
   if (otype == 40) then         ! TEMPERATURE_2M
      read(input_line(3:129), *, iostat=rcio) lat, lon, vert, &
                                ...
                                temp, terr
      call create_3d_obs(lat, lon, vert, VERTISSURFACE, temp, &
   ...
   elseif (otype == 41 .or. otype == 42) then  ! U_WIND_10M or V_WIND_10M
      read(input_line(3:129), *, iostat=rcio) lat, lon, vert, &
                                year, month, day, hour, minute, second, &
                                wind, werr
      call create_3d_obs(lat, lon, vert, VERTISSURFACE, wind, &
   ...
   elseif (otype == 43) then     ! SURFACE_PRESSURE
      ...
      call create_3d_obs(lat, lon, vert, VERTISSURFACE, pres, &
   ...
   elseif (otype == 83) then     ! LAND_SFC_RELATIVE_HUMIDITY
      read(input_line(3:129), *, iostat=rcio) lat, lon, vert, &
                                year, month, day, hour, minute, second, &
                                rh, rherr
      call create_3d_obs(lat, lon, vert, VERTISSURFACE, rh, &
