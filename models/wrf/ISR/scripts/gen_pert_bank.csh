...
set num_ens =  100  # number of perturbations to generate, must be at least ensemble size, suggest 3-4X. SUGGEST testing
                    # a single member until you are sure the script works, and are happy with the settings.
set wrfin_dir = ${template_dir}#work_dir}/template
...
#SBATCH --nodes=1
#SBATCH --cpus-per-task=4  # Request 2 CPUs for this task
#SBATCH --ntasks=1         # This is one task
...
srun ./da_wrfvar.exe >& output.wrfvar
...
EOF
   ...
   sbatch ${work_dir}/mem_${n}/gen_pert_${n}.csh
   @ n++
end
