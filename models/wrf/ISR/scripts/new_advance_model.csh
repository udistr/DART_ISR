...
# may want multiple lists here, e.g. do we want w from the analysis?
  ...
  while ( $dn <= $num_domains )
    ...
    if ( -e ../${this_file} ) then
      /shared/miniconda3/envs/gcc_env/bin/ncks -A -v ${stuff_str} ../${this_file} wrfinput_d${dchar}
    else
...
# DMODS - note the wrf.info file was pre-generated, not from dart_to_wrf
  ...
  set ADV_MOD_COMMAND   = `head -n 6 wrf.info | tail -n 1`
...
EOF
    ...
    if ( -e ${CENTRALDIR}/append_precip_to_diag ) then
       ...
       while ( $dn <= $num_domains )
          /shared/miniconda3/envs/gcc_env/bin/ncks -h -O -F -v RAINC,RAINNC wrfout_d0${dn}_${END_STRING} wrf_precip.nc
