...
while ( 1 == 1 )
   ...
   if ( $SUPER_PLATFORM == 'LSF queuing system' ) then
      ...
      set ic_queue = "compute"
      set sub_command = "sbatch --nodes=1 --ntasks=128 --mem=5G --partition=${ic_queue} --output=%x-%j.out --job-name=icgen"
   endif

   echo "this platform is $SUPER_PLATFORM"
   ...
   /shared/miniconda3/envs/gcc_env/bin/ncdiff -F -O -v $extract_str postassim_mean.nc preassim_mean.nc analysis_increment.nc
   /shared/miniconda3/envs/gcc_env/bin/ncks -F -O -x -v ${extract_str} postassim_mean.nc static_data.nc
   /shared/miniconda3/envs/gcc_env/bin/ncks -A static_data.nc analysis_increment.nc
         ...
         while ( ! -e ${RUN_DIR}/start_member_${n} )
            ...
            if ( $SUPER_PLATFORM == 'LSF queuing system' ) then
               ...
               if ( `qstat -wa | grep assim_advance_${n} | wc -l` == 0 ) then
                ...
                if ( `squeue -n assim_advance_${n} | wc -l` == 1 ) then
                    echo "Warning, detected that assim_advance_${n} is missing from the queue"
                    echo "If this warning leads to missing output from ensemble ${n}"
                    echo "consider enabling the sbatch command within keep_trying while statement in driver.csh"
