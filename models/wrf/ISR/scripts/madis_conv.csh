...
set src_base_dir = /shared/DART/MADIS
#set HH = `printf %02d $start_hour`
#set DATE = `echo ${2}${3}${4}${HH} +3h | ./advance_time`
set out_dir = ./data_madis/ #/shared/DART/ISR/output/${DATE}/MADIS/
...
while ( $h <= $totalhrs )
  ...
  set point_list = (acars acarsProfiles HDW HDW1h maritime metar POES profiler radiometer raob sao satrad)

  # append YYYYMM/DD/type to the base directory
  if (`echo $point_list | grep -w $type2` != "") then
      set nc="netcdf"
      set src_dir = ${src_base_dir}/point/${type2}/${nc}/
  else
      set nc="netCDF"
      set src_dir = ${src_base_dir}/LDAD/${type2}/${nc}/
  endif

  # input and output filenames (without paths)
  set infn  = ${year}${month}${day}_${hour}00

  if (! -e $src_dir/$infn) then
    if ($type == "satwnd") then
      set src_dir = ${src_base_dir}/point/HDW/${nc}/
    endif
  endif

  if ($daily == true) then
    set outfn = obs_seq_${type}
  else
    set outfn = obs_seq_${type}
  endif
